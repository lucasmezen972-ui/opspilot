...
export const setOfflineTasks = async (tasks: any[]) => {
  ...
  db.transaction(tx => {
    ...
    tasks.forEach(t => {
      tx.executeSql('INSERT INTO tasks (data, synced) VALUES (?, 1)', [JSON.stringify(t)])
    })
  })
}
...
export const queueTask = async (task: any) => {
  if (!db) return
  db.transaction(tx => {
    tx.executeSql('INSERT INTO tasks (data, synced) VALUES (?, 0)', [JSON.stringify(task)])
  })
}
...
export const setOfflineAudits = async (audits: any[]) => {
  ...
  db.transaction(tx => {
    ...
    audits.forEach(a => {
      tx.executeSql('INSERT INTO audits (data, synced) VALUES (?, 1)', [JSON.stringify(a)])
    })
  })
}
...
export const queueAudit = async (audit: any) => {
  if (!db) return
  db.transaction(tx => {
    tx.executeSql('INSERT INTO audits (data, synced) VALUES (?, 0)', [JSON.stringify(audit)])
  })
}
...
export const queuePhoto = async (auditId: string, uri: string) => {
  if (!db) return
  db.transaction(tx => {
    tx.executeSql('INSERT INTO photos (uri, audit_id, synced) VALUES (?, ?, 0)', [uri, auditId])
  })
}
...
const updatePhotoAuditIds = (oldId: string, newId: string) => {
  if (!db) return
  db.transaction(tx => {
    tx.executeSql('UPDATE photos SET audit_id = ? WHERE audit_id = ?', [newId, oldId])
  })
}
...
const getUnsynced = (table: string): Promise<any[]> => runQuery(`SELECT * FROM ${table} WHERE synced = 0`)
      ...
      if (!uploadError) {
        ...
        if (publicUrl?.publicUrl) {
          const { data: auditData } = await supabase
            ...
            .eq('id', p.audit_id)
            .single()
          const updatedPhotos = auditData?.photos ? [...auditData.photos, publicUrl.publicUrl] : [publicUrl.publicUrl]
          await supabase
            .from('audits')
