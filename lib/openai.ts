...
export const analyzeAuditImage = async (imageUrl: string, auditType: string = 'general'): Promise<AuditAnalysis> => {
  try {
...
export const generateTrainingContent = async (topic: string, difficulty: 'beginner' | 'intermediate' | 'advanced'): Promise<TrainingContent> => {
  try {
    const response = await openai.chat.completions.create({
      model: "gpt-4",
      messages: [
        {
          role: "system",
          content: "Vous êtes un expert en formation pour le secteur de la grande distribution et retail. Créez du contenu de formation professionnel et pratique."
        },
        {
          role: "user",
...
export const getChatAssistantResponse = async (userMessage: string, context: string = ''): Promise<string> => {
  try {
    const response = await openai.chat.completions.create({
      model: "gpt-4",
      messages: [
